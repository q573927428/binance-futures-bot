...
/**
 * 从币安获取历史K线数据脚本（修复时区显示问题）
 * 这个脚本用于从币安获取22000条历史K线数据
 * 注意：币安API使用UTC时间，所有时间戳都是UTC时间
 */
...
// 格式化时间显示，同时显示UTC和北京时间
function formatTimeDisplay(timestampMs) {
  const date = new Date(timestampMs);
  const utcTime = date.toUTCString();
  const beijingTime = date.toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });
  return `UTC: ${utcTime} | 北京时间: ${beijingTime}`;
}
...
async function fetchHistoryData(symbol, timeframe, totalBars = 22000, batchSize = 1000) {
  ...
  console.log(`📌 注意：币安API使用UTC时间，所有时间戳都是UTC时间`);
  console.log(`📌 北京时间 = UTC时间 + 8小时\n`);
  ...
  console.log(`   📅 上线时间: ${formatTimeDisplay(symbolInfo.onboardDate)}`);
  ...
  console.log(`\n📊 时间范围分析:`);
  console.log(`   当前时间: ${formatTimeDisplay(now * 1000)}`);
  console.log(`   理论开始时间: ${formatTimeDisplay(theoreticalStartTimestamp * 1000)}`);
  console.log(`   实际上线时间: ${formatTimeDisplay(onboardTimestamp * 1000)}`);
  console.log(`   实际开始时间: ${formatTimeDisplay(actualStartTimestamp * 1000)}`);
  ...
  try {
    while (fetchedBars < actualTargetBars && consecutiveEmptyBatches < maxConsecutiveEmptyBatches) {
      ...
      console.log(`   开始时间: ${formatTimeDisplay(currentStartTime * 1000)}`);
      ...
      try {
        ...
        if (batchData.length === 0) {
          ...
          console.log(`   ⏰ 跳过空数据区间，下一个开始时间: ${formatTimeDisplay(currentStartTime * 1000)}`);
        } else {
          ...
          if (batchData.length > 0) {
            ...
            console.log(`   📅 数据时间范围:`);
            console.log(`       第一条: ${formatTimeDisplay(firstItem.timestamp * 1000)}`);
            console.log(`       最后一条: ${formatTimeDisplay(lastItem.timestamp * 1000)}`);
          }
          ...
          if (lastItem) {
            ...
            console.log(`   ⏰ 下一个批次开始时间: ${formatTimeDisplay(currentStartTime * 1000)}`);
          } else {
        ...
        // 跳过一段时间继续尝试
        currentStartTime = batchEndTime + 1;
        console.log(`   ⏰ 跳过失败区间，下一个开始时间: ${formatTimeDisplay(currentStartTime * 1000)}`);
      }
    ...
    if (allData.length > 0) {
      ...
      console.log(`\n📅 数据时间范围:`);
      console.log(`   开始时间: ${formatTimeDisplay(firstItem.timestamp * 1000)}`);
      console.log(`   结束时间: ${formatTimeDisplay(lastItem.timestamp * 1000)}`);
      console.log(`\n📁 文件位置: data/kline-simple/${symbol.replace('/', '')}-${timeframe}.json`);
